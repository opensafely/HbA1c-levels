...
def gen_median(df_in, group=''):
    if group == '':
        df_med = df_in[['date','hba1c_val_58_74','hba1c_val_75']].groupby(['date']).median().reset_index()
        df_ct_58_74 = df_in[['date','hba1c_val_58_74']].groupby(['date']).count().reset_index().rename(
            columns={'hba1c_val_58_74':'ct_58_74'})
        df_ct_75 = df_in[['date','hba1c_val_75']].groupby(['date']).count().reset_index().rename(
            columns={'hba1c_val_75':'ct_75'})
        df_out = reduce(lambda left,right: pd.merge(left,right,on=['date']), 
                        [df_med, df_ct_58_74, df_ct_75])

    else:
        df_med = df_in[[group] + ['date','hba1c_val_58_74','hba1c_val_75']].groupby(['date', group]).median().reset_index()
        df_ct_58_74 = df_in[[group] + ['date','hba1c_val_58_74']].groupby(['date', group]).count().reset_index().rename(
            columns={'hba1c_val_58_74':'ct_58_74'})
        df_ct_75 = df_in[[group] + ['date','hba1c_val_75']].groupby(['date', group]).count().reset_index().rename(
            columns={'hba1c_val_75':'ct_75'})
...
# Export data
df_med_t2dm.to_csv('output/data/input_med_t2dm_all.csv')
df_med_t2dm_age.to_csv('output/data/input_med_t2dm_age.csv')
df_med_t2dm_sex.to_csv('output/data/input_med_t2dm_sex.csv')
df_med_t2dm_eth.to_csv('output/data/input_med_t2dm_eth.csv')
df_med_t2dm_reg.to_csv('output/data/input_med_t2dm_reg.csv')
df_med_t2dm_imd.to_csv('output/data/input_med_t2dm_imd.csv')
df_med_t2dm_ld.to_csv('output/data/input_med_t2dm_ld.csv')
df_med_t2dm_mi.to_csv('output/data/input_med_t2dm_mi.csv')